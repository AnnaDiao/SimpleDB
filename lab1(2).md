...
bug fixes, new commits to the repo, etc., as bugs and issues are reported.

## 0. Environment Setup

Start by downloading the code for lab from the course GitHub repository [NKU-DBIS-DB/SimpleDB](https://github.com/NKU-DBIS-DB/SimpleDB) or the QQ Group. Then create a private repository called "SimpleDB" on GitHub and initialize the repository from the code.

Another way is using GitHub Import. Create a private repository called "SimpleDB" and import the code from the course GitHub repository [NKU-DBIS-DB/SimpleDB](https://github.com/NKU-DBIS-DB/SimpleDB).
...
<tt>getNumEmptySlots()</tt> and <tt>isSlotUsed()</tt> in HeapPage. These require pushing
...
are iterator based; each operator implements the `DbFileIterator`
interface.
...
`next` on the root operator; this operator then calls
`next` on its children, and so on, until these leaf operators
are called.  They fetch tuples from disk and pass them up the tree (as return
arguments to `next`); tuples propagate up the plan in this way
...
Place the write-up in a file called <tt>学号+姓名+Lab1.txt</tt> or 
<tt>学号+姓名+Lab1.pdf</tt> in the top level of your <tt>SimpleDB</tt> directory.  **Important:** 
In order for your write-up to be added to the git repo, you need to explicitly add it:

```bash
$ git add 学号+姓名+Lab1.txt
```
...
There is a bash script `turnInLab1.sh` in the root level directory of SimpleDB that commits 
...
To git@github.com:NKU-DBIS-DB/SimpleDB.git
 - [deleted]         lab1
...
6. The last thing that we strongly recommend you do is to go to GitHub to
   make sure that your solutions has been uploaded.
...
在跑测试代码的时候，我们会从 GitHub 上拉取大家提交后的代码，并将 build.xml 测试代码替换成最初的版本。如果你对程序的 API 做了任何改动导致无法通过测试，一定要在作业报告和代码讲解的过程中主动提出来。

- 50% 源代码（通过测试，代码风格）

- 10% Git Commit History（截图放在报告中）

- 20% 作业报告（给出 GitHub 地址，思考题，设计思路，重难点，改动部分）

- 20% 代码讲解

代码讲解之前请将源代码和作业报告打包，压缩文件名为“学号+姓名+Lab1”，讲解时交给助教，留作备份。

代码讲解的时间原本安排在截止时间后两周内，由于疫情影响，代码讲解等返校后再做统一安排。
